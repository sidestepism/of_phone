...
void testApp::setup(){
	...
	soundStream.setup(this, 0, 1, 48000, bufferSize, 4);
}
...
void testApp::draw(){
    ...
    for (int i = 0; i < recBufferSize; i++){
        ofVertex(i*2, 100 -recTemp[i]*180.0f);
    }
    ofEndShape(false);
...
void testApp::audioIn(float * input, int bufferSize, int nChannels){
	...
	for (int i = 0; i < bufferSize; i++){
		recTemp[i]		= input[i]*0.5;
		curVol += recTemp[i] * recTemp[i];
		numCounted+=1;
	}
    ...
    for (int i = 0; i < bufferSize; i+= 6){
        ...
        recBuffer[recBufferCounter++] = (unsigned short) (128 + (256 * recTemp[i]) * 4);
    }
...
void testApp::audioOut(float * output, int bufferSize, int nChannels){
    ...
    for (int i = 0; i < bufferSize; i++){
        frame = floor(i / 6);
        float sample = ((float)(playBuffer[frame]) - 128) / 256;
